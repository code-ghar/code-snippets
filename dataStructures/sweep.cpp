...
public:
    ...
    Sweep(int expirySize) {
        currPos=0;
        expiry=vector<vector<int>>(expirySize); // for BST not initilization needed
    }

    void add(int value, int exp) {
        valueOpenIntervals.insert(value);
        if(exp<expiry.size()) expiry[exp].push_back(value);
    }

    int get(int time) {
        while(currPos<time) {
            if(expiry[currPos].empty()) currPos++;
            else {
                for(auto x:expiry[currPos]) 
                    valueOpenIntervals.erase(valueOpenIntervals.find(x));
                expiry[currPos].clear();
            }
        }
        if(valueOpenIntervals.empty()) return inf;
        return *valueOpenIntervals.begin(); 
    }
};
