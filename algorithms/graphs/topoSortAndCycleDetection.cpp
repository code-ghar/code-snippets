...
bool dfs(int node, vector<int> &topo, vector<vector<int>> &adj, vector<int> &vis){
    vis[node]=2;
    bool cyclic=false;
    for(auto child:adj[node]){
        if(vis[child]==0) cyclic|=dfs(child,topo, adj, vis);
        if(vis[child]==2) cyclic=true;
    }
    vis[node]=1;
    topo.push_back(node);
    return cyclic;
...
//khans algo
vector<int> topoSortCycleDetectionBfs(vector<vector<int>> &adj) {
    int n=adj.size();
