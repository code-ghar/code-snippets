...
vector<vector<vector<int>>> kruskalsMST(vector<vector<vector<int>>> &graph) {
    int V=graph.size();
    vector<vector<vector<int>>> tree(V);
    ...
    for(auto x:edges){
        int wt=x[0], node=x[1], child=x[2];
        if(dsu.findSet(node) != dsu.findSet(child)){
            tree[node].push_back({child, wt});
            tree[child].push_back({node, wt});
...
vector<vector<vector<int>>> primsMST(vector<vector<vector<int>>> &graph) {
    int V=graph.size();
    vector<vector<vector<int>>> tree(V);
